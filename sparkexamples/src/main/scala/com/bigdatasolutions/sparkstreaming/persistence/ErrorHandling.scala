package com.bigdatasolutions.sparkstreaming.persistence

import com.madhukaraphatak.sparktraining.streaming.serialization.SalesRecordParser
...
object ErrorHandling {
  ...
  def main(args: Array[String]) {
    ...
    def erroCountUpdateFunction(rows:Seq[String], runningValue:Option[Double]) = {
      val newValue = rows.length + runningValue.getOrElse(0.0)
      Some(newValue)
    }
    ...
    val errorCount = malformedRecords.map(row => (null,row)).updateStateByKey(erroCountUpdateFunction _).map(_._2)
